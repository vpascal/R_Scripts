# There are a number of components that have to changed manually. Plese search for "manually," to see what/where things need to be changed.
...
data <- read.delim("report_2014-06-03.csv", header = T, sep = "\t")
...
webpage <-webpage[which(webpage$webpage.Date =="2014-02-01"|webpage$webpage.Date =="2014-01-01"),]
# webPage visits
...
consultations_type_feb <- consultations_type[consultations_type$Date == "2014-02-01", ]
...
plot(a, col = "#00A6FF", border = "white", lwd = 0.4)
raions <- c(unique(data$Raion))
network <- a[a$NAME_1 %in% raions, ]
plot(network, border = "white", col = "orange", add = T, lwd = 0.4)
...
title("Number of Library Users (in '000)", font = 2)
...
mtext("Virtual visits in February", side = 3, line = -2, cex = 1.2, col = "darkred")
##################################### 
